...
aspot.DB = function() {
  that = {}
  that.variable = aspot.DB.variable;
  that.parseQuery = aspot.DB.parseQuery;
  return that;
}
aspot.DB.parseQuery = function(query, ret, currentVariable) {
  data = {};
  data.current = typeof currentVariable === 'undefined' ? aspot.DB.variable() : currentVariable;
  data.query = query;
  data.ret = typeof ret === 'undefined' ? [] : ret;
  operators = ['='];
  trail = ['\.'];
  aspot.DB.parseQuery.attrChain(data);
  if (data.query.length >0) {
    data.ret = aspot.DB.parseQuery(data.query, data.ret, data.current);
  }
  return data.ret;
}
/*
aspot.DB.parseQuery.attrChain = function(data) {
  rx = /(.*)((?:\.)\((?:=)\)"[^"]*")$/;
...
aspot.localTestDB = function(store) {
  var that = {};
  that.store = {};
 ...
 return that;
 // return new aspot.localTestDB.LocalTestDB(store);
}
...
aspot.query = function(query) {
  ...
  that.compile = function() {
    return this.trail.compile(aspot.query.instruction.variable()).instruction;
  }
  return that;
}
