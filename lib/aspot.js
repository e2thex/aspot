...
aspot.DB = function() {
  ...
  that.query = function(query) {
    return this.retrieve(aspot.query(query).compile())
  }
...
aspot.localDB = function(store) {
 ...
 that.requestLOOKUP = function (instruction) {
   var load = {}
   var variables = {}
   if(typeof instruction.predicate.name !== 'undefined') {
     variables[instruction.predicate.name] = 'predicate';
   }
   else {
     load.predicate = instruction.predicate.value;
   }
   if(typeof instruction.subject.name !== 'undefined') {
     variables[instruction.subject.name] = 'subject';
   }
   else {
     load.subject = instruction.subject.value;
   }
   if(typeof instruction.object.name !== 'undefined') {
     variables[instruction.object.name] = 'object';
   }
   else {
     load.object = instruction.object.value;
   }
 ...
 that.retrieve = function(request) {
   var table = this.requestLOOKUP(request.instruction);
   var db = this;
   return table.table.map(function(row) {  return db.datum(row[request.variable.name])})
 }
 return that;
...
aspot.query.instruction.lookupPart = function(op, value) {
  var that = {}
  that.operator = op;
  that.value = value;
  return that;
