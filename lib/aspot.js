...
aspot.query.trail = function(query, type) {
  ...
  if(parse = /^(\.|\..)"(.*?)"(.*)$/.exec(query)) { 
    ...
    var trail = parse[2];
    trail_item = aspot.query.getSubjectFromEqualPredicate(getSubjectFromEqualPredicate);
    query = trail;
  }
...
aspot.token.trailCompare = function(trail, operator, value) {
  ...
  that.compile = function(variable) {
    ...
    var rhs = aspot.instruction.lookup(
      v1,
      v2,
      aspot.instruction.lookupPart(this.operator, this.value, lhsc.variable)
    );
    return {
