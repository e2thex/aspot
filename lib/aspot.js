 ...
 *  * Trail_Item                  : getSubjectFromEqualPredicate
 *    Trail_Item                  : getFromPredicate
 *    Trail_Item                  : getPredicate
 *  * Trail_Item                  : [Where_Item]
 *  * getObjectFromEqualPredicate : ."STRING"  (this should be part of getFromPrediate)
 *  * getSubjectFromEqualPredicate: .<"STRING" (this should be part of getFromPrediate)
 *    getFromPredicate            : .>"STRING"
 *    getFromPredicate            : .><"STRING"
 *    getFromPredicate            : .{Where_Item}
 *    getFromPredicate            : .>{Where_Item}
 *    getFromPredicate            : .<{Where_Item}
 *    getFromPredicate            : .><{Where_Item}
 *    getFromPredicate            : .><{Where_Item}
 *    getPredicate                : @
 *  * Where_Item                  : trailExists
 *  * Where_Item                  : trailCompare
 *    Where_Item                  : Where_Item AND Where_Item
 *    Where_Item                  : Where_Item OR Where_Item
...
aspot.token.getFromPredicateValue = function(value, direction) {
  ...
  that.compile = function(variable) {
    ...
    var instruction = aspot.query.instruction.lookup(
      v0,
      aspot.query.instruction.lookupPart("=", this.value),
      v1
    );
...
aspot.query.getSubjectFromEqualPredicate = function(query) {
  rx = /^\.<"(.*)"$/;
  parse = rx.exec(query);
  if(parse) {
    var that = {};
    ...
    throw new Error("PARSE ERROR: " + query + " is not a valid get Subject from equal predicate attr trail");
  }
  that.compile = function(variable) {
    var newVar = variable.next()
    var instruction = aspot.query.instruction.lookup(
...
aspot.query.trail = function(query, type) {
  ...
  if(parse = /^(\.|\..)"(.*?)"(.*)$/.exec(query)) { 
    ...
    var value = parse[2];
    var trail = parse[3];
    that.item = aspot.token.getFromPredicateValue(value, direction);
    query = trail;
  }
  else if(parse = /^(\.<".*?")(.*)$/.exec(query)) { 
    var getSubjectFromEqualPredicate = parse[1];
    var trail = parse[2];
    that.item = aspot.query.getSubjectFromEqualPredicate(getSubjectFromEqualPredicate);
    query = trail;
  }
...
aspot.query.where = function(query) {
  ...
  if(parse = /^(.*) EXISTS$/.exec(query)) { 
    ...
    that.item = aspot.token.trailExists(trail);
  }
  else if(parse = /^(.*) (=|contains) "(.*)"$/.exec(query)) { 
    var operator = parse[2];
    var value = parse[3];
    var trail = aspot.query.trail(parse[1]);
    that.item = aspot.token.trailCompare(trail, operator, value);
  }
  else {
